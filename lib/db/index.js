...
export async function createBlog(blogData) {
  ...
  if (db) {
    try {
      const insertResult = await db.collection('blogs').insertOne({ ...newBlog });
      if (insertResult?.insertedId) {
        newBlog._id = insertResult.insertedId.toString();
      }
    } catch (err) {
      console.error('MongoDB createBlog error:', err);
      throw new Error(`Database error saving article: ${err.message}`);
    }
  } else if (process.env.MONGODB_URI) {
    throw new Error('Database connection failed. Please ensure MongoDB Atlas Network Access allows connections (0.0.0.0/0).');
  }
...
export async function createResource(resourceData) {
  ...
  if (db) {
    try {
      const insertResult = await db.collection('resources').insertOne({ ...newResource });
      if (insertResult?.insertedId) {
        newResource._id = insertResult.insertedId.toString();
      }
    } catch (err) {
      console.error('MongoDB createResource error:', err);
      throw new Error(`Database error saving resource: ${err.message}`);
    }
  } else if (process.env.MONGODB_URI) {
    throw new Error('Database connection failed. Please ensure MongoDB Atlas Network Access allows connections (0.0.0.0/0).');
  }
